...
export const fetchContactsApi = () => {
  return new Promise((resolve, reject) => {
    const records = `{"latitude": "52.986375", "user_id": 12, "name": "Christina McArdle", "longitude": "-6.043701"}
...
let recordsSplitByEndLine = records.split('\n');
recordsSplitByEndLine = recordsSplitByEndLine.map(item => JSON.parse(item));
    resolve(recordsSplitByEndLine);
  });
}